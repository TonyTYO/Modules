...
class Textwrap:
    ...
    """
    __slots__ = ['base', 'flags', 'current', 'surface_dicts', 'surface_sizes', 'surface_dict', 'curr_line', 'adjust']

    def __init__(self, rect, font, text_colour=WHITE,
                 background_colour=BLACK, justification=0, frame=0, frame_colour=BLACK, adjust=False):
        """ Create the text box inside which text will be printed
            ...
            adjust: Adjust height to fit contents

        """
        # Base values for instance stored in dictionary as follows
        # font:font object, base_font:font specified in print_text()
        ...
        self.base = {"font": font, "base_font": font,
    ...
    def _get_size(self):
        ...
        if self.adjust:
            self.base["rect"].height = self.total_height()

    def total_height(self):
        height = 0
        min_height = 1000
        if self.surface_sizes:
            for dictionary in self.surface_sizes:
                height += dictionary[-1][1]
                if min_height > dictionary[-1][1]:
                    min_height = dictionary[-1][1]
            return int(height + 2 * min_height)
        return height
    ...
    def _do_frame(self, surface):
        ...
        if self.base["frame"] > 0:
            ...
            self._draw_frame(surface, self.base["frame_colour"], frame_rect, frame, self.base["background_colour"])
